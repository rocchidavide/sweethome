...
from core.constants import COMMAND_INSTALL, COMMAND_REMOVE, COMMAND_DOT_ALL
...
if __name__ == "__main__":
    ...
    parser_install = subparsers.add_parser(
        COMMAND_INSTALL,
        help="Install all dots listed or the one specified"
    )
    parser_install.add_argument(
        "dot",
        type=str,
        nargs="?",
        default=COMMAND_DOT_ALL,
        help="dot's name to be installed"
    )
    parser_install.add_argument(
        "--force",
        action="store_true",
    ...
    parser_remove = subparsers.add_parser(
        COMMAND_REMOVE,
        help="Remove all dots listed or the one specified"
    )
    ...
    if args_dot_name != COMMAND_DOT_ALL and config.get_dot_path(args_dot_name) is None:
        print(f"\n{Colors.WARNING}➱ Error: dot {args_dot_name} not found!{Colors.ENDC}")
        sys.exit()
    ...
    if args_dot_name == COMMAND_DOT_ALL:
